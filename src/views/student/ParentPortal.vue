...
<template>
  <div class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div v-if="!studentData" class="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden">
          <div class="bg-brand-primary p-8 text-center text-white relative">
              ...
              <p class="text-rose-100 mt-2">Track your child's progress</p>
          </div>
          <div class="p-8">
              <form @submit.prevent="checkResults">
                  <div class="mb-6">
                      <label class="block text-gray-700 font-medium mb-2" for="code">Parent Code</label>
                      <input id="code" type="text" v-model="parentCode" class="w-full border-2 border-gray-200 rounded-lg p-3 focus:outline-none focus:border-brand-primary font-mono tracking-widest text-center uppercase" placeholder="PRNT-123456" />
                  </div>
                  ...
                  <button type="submit" :disabled="loading" class="w-full bg-brand-primary text-white font-bold p-3 rounded-lg hover:bg-brand-secondary transition flex items-center justify-center">
      ...
      <div v-if="studentData" class="max-w-4xl w-full bg-white rounded-2xl shadow-xl p-8">
          <div class="flex justify-between items-start mb-8 pb-8 border-b">
              <div>
                  ...
                  <p class="text-gray-500 mt-1">Current Level: <span class="font-bold text-brand-primary">{{ studentData.student_level || 'Mapping in progress' }}</span></p>
              </div>
              ...
              <div v-for="(attempt, index) in studentData.attempts" :key="attempt.id" class="border rounded-xl p-6 hover:shadow-md transition bg-gray-50">
                  <div class="flex justify-between items-center mb-4">
                      ...
                      <div class="text-center">
                          <div class="text-3xl font-black text-brand-primary">{{ attempt.overall_score || 0 }}</div>
                  ...
                  <div class="bg-rose-50 p-4 rounded-lg flex items-center justify-between">
                     <span class="text-sm font-medium text-indigo-800">Proctoring Status Verified</span>
                     <button class="bg-brand-primary text-white text-xs px-3 py-1 font-bold rounded hover:bg-brand-secondary">Download PDF</button>
                  </div>
              </div>
          </div>
      </div>
