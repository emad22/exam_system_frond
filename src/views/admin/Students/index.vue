    ...
    <AdminLayout>
        <div class="space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-1000 pb-24 mt-8 px-4 md:px-12">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6 md:space-y-0">
                <div>
                    <h1 class="text-3xl font-black text-slate-800 tracking-tight lowercase first-letter:uppercase">Candidate Registry</h1>
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <Button v-if="selectedStudents.length > 0" label="Purge Selected" icon="pi pi-trash" severity="danger" class="text-[10px] font-black uppercase tracking-widest px-6" @click="bulkDelete" />
                    <Button label="Bulk Skills" icon="pi pi-tags" severity="secondary" outlined class="text-[10px] font-black uppercase tracking-widest px-6" @click="showBulkSkillsModal = true" />
                    <Button label="Matrix Import" icon="pi pi-file-excel" severity="secondary" outlined class="text-[10px] font-black uppercase tracking-widest px-6" @click="router.push('/admin/students/batch')" />
                    <Button label="Register Entity" icon="pi pi-plus" class="bg-brand-primary border-none text-[10px] font-black uppercase tracking-widest px-8 shadow-lg shadow-rose-100 transition-all hover:-translate-y-1" @click="router.push('/admin/students/create')" />
                </div>
            </div>
                ...
                <template #content>
                    <DataTable :value="filteredStudents" v-model:selection="selectedStudents" dataKey="id" paginator :rows="10" 
                        ...
                        <template #header>
                            <div class="flex justify-end p-2 pb-4">
                                <span class="relative">
                                    ...
                                    <InputText v-model="searchQuery" placeholder="Search identities..." class="pl-10 w-full md:w-80 shadow-sm rounded-xl border-slate-100 font-bold" />
                                </span>
                            </div>
                        </template>
                        ...
                        <Column header="Institutional Identity" style="min-width: 280px">
                            <template #body="{ data }">
                                <div class="flex items-center space-x-4">
                                    <div class="w-11 h-11 rounded-2xl bg-slate-50 text-brand-primary flex items-center justify-center font-black border border-slate-100">
                                        {{ data.user?.first_name ? data.user.first_name[0].toUpperCase() : 'S' }}
                                    </div>
                                    <div>
                                        <div @click="openView(data)" class="font-black text-slate-800 hover:text-brand-primary cursor-pointer uppercase tracking-tight transition-colors">
                        ...
                        <Column header="Subscription" style="min-width: 150px">
                            <template #body="{ data }">
                                <Tag v-if="data.package" :value="data.package.name" severity="info" class="text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-lg" />
                                <span v-else class="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] italic">Custom Asset</span>
                            </template>
                        </Column>

                        <Column header="Logic Stream" style="min-width: 150px">
                            <template #body="{ data }">
                                <div class="flex flex-col space-y-1">
                                    <Tag :value="data.category?.name || 'GENERIC'" 
                                         :severity="data.category?.slug === 'adult' ? 'success' : 'info'" 
                                         class="text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-lg border-none" />
                                    <div v-if="data.not_adaptive" class="text-[8px] font-black text-brand-accent uppercase tracking-tighter">Non-Adaptive</div>
                                </div>
                            </template>
                        </Column>

                        <Column header="Origin" style="min-width: 120px" class="text-center">
                            <template #body="{ data }">
                                <Tag :value="data.registration_source || 'Internal'" severity="secondary" class="text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-lg" />
                            </template>
                        </Column>

                        <Column header="Integrity" style="min-width: 100px" class="text-center">
                            <template #body="{ data }">
                                <i :class="data.user?.is_active ? 'pi pi-check-circle text-emerald-500' : 'pi pi-times-circle text-rose-300'" class="text-lg"></i>
                            </template>
                        </Column>
                        ...
                        <template #empty>
                            <div class="p-12 text-center text-slate-300 font-bold uppercase tracking-widest text-xs italic">No candidate identities discovered.</div>
                        </template>
                    </DataTable>
                </template>
            </Card>
        ...
        <Dialog v-model:visible="showBulkSkillsModal" 
                :header="'Institutional Sync • Bulk Mapping'" 
                :style="{ width: '500px' }"
                modal 
            ...
            <template #header>
                <div class="flex flex-col">
                    ...
                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Bulk module mapping protocol</p>
                </div>
            </template>

            <div class="pt-6 space-y-8">
                
                <div class="bg-rose-50/50 border border-brand-primary/10 p-5 rounded-2xl text-[10px] font-bold text-brand-primary leading-relaxed uppercase tracking-wider">
                    <i class="pi pi-info-circle mr-2"></i>
                    Update module assignments either by pasting emails manually, OR by uploading the master sheet.
                </div>

                <div class="space-y-6">
                    <div class="bg-slate-50/50 p-6 rounded-2xl border border-slate-100 flex flex-col items-center space-y-4">
                        <Button label="Master Sheet Template" icon="pi pi-download" size="small" text @click="downloadExcelTemplate" class="text-[10px] uppercase font-black tracking-widest text-brand-primary" />
                    ...
                    <div class="relative flex items-center justify-center">
                        <div class="absolute inset-0 flex items-center px-4"><div class="w-full border-t border-slate-100"></div></div>
                        <span class="relative bg-white px-4 text-[9px] font-black text-slate-300 uppercase tracking-widest">Manual Override</span>
                    </div>

                    <div class="space-y-4">
                        <div class="space-y-2">
                             <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Identity Access Emails</label>
                             <textarea v-model="bulkEmails" 
                                ...
                                placeholder="E.G. JOHN@DOMAIN.COM..."></textarea>
                        </div>
                        <div class="space-y-2">
            ...
            <template #footer>
                <div class="flex justify-end pt-6 border-t border-slate-50 space-x-3">
                    <Button label="Discard" outlined severity="secondary" class="text-[10px] font-black uppercase tracking-widest px-8" @click="showBulkSkillsModal = false" />
                    <Button :label="isBulkSaving ? 'SYNCING...' : 'COMMIT SYNC'" :loading="isBulkSaving" class="bg-brand-primary border-none text-[10px] font-black uppercase tracking-widest px-8 shadow-lg shadow-rose-100" @click="submitBulkSkills" />
                </div>
            </template>
        </Dialog>
    </AdminLayout>
