...
const saveExam = async () => {
    ...
    try {
        ...
        for (const sId in localQuestions.value) {
            for (const lId in localQuestions.value[sId]) {
                const passageMap = {}; // To reuse newly created passages within same skill/level

                for (const q of localQuestions.value[sId][lId]) {
                    if (!q.id) {
                        // It's a new question, save it to the bank first
                        const formData = new FormData();
                        formData.append('skill_id', sId);
                        formData.append('level_id', lId); // 1-9
                        formData.append('type', q.type);
                        formData.append('content', q.content);
                        formData.append('instructions', q.instructions || '');
                        formData.append('points', q.points);
                        formData.append('media_file', q.media_path_file || ''); // Handle file if present

                        if (q.passage_mode !== 'none') {
                            formData.append('passage_mode', q.passage_mode);
                            if (q.passage_mode === 'new') {
                                // If we already created this passage group in this skill/level, use 'existing' for subsequent questions
                                if (q.temp_group_id && passageMap[q.temp_group_id]) {
                                    formData.set('passage_mode', 'existing');
                                    formData.append('passage_id', passageMap[q.temp_group_id]);
                                } else {
                                    formData.append('passage_title', q.passage_title || 'Passage for ' + form.value.title);
                                    formData.append('passage_content', q.passage_content);
                                    formData.append('passage_type', q.passage_type || 'text');
                                    formData.append('is_random', q.passage_randomize ? 1 : 0);
                                    formData.append('questions_limit', q.passage_limit || 5);
                                    if (q.passage_media_file) formData.append('passage_file', q.passage_media_file);
                                }
                            } else if (q.passage_mode === 'existing') {
                                formData.append('passage_id', q.passage_id);
                            }
                        }
                        ...
                        const res = await api.post('/admin/questions', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
                        q.id = res.data.question.id;

                        // If this was a new passage, remember its ID for the rest of the group
                        if (q.passage_mode === 'new' && res.data.question.passage_id && q.temp_group_id) {
                            passageMap[q.temp_group_id] = res.data.question.passage_id;
                        }
                    }
        ...
        const skillsWithRules = form.value.selectedSkills.map(s => {
            ...
            if (localQuestions.value[s.skill_id]) {
                Object.entries(localQuestions.value[s.skill_id]).forEach(([lvlId, qs]) => {
                    if (qs.length > 0) {
                        rules.push({
        ...
        <div v-else
                    ...
                    <div class="flex items-center space-x-6">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1 italic">Phase
                                {{ currentStep }} of 4: Protocol Construction</p>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="space-y-8">
                            <div class="space-y-3">
                                ...
                                <input v-model="form.title" type="text"
                                    class="w-full bg-slate-50 border border-slate-100 p-6 rounded-[1.5rem] text-sm font-bold uppercase tracking-tight focus:bg-white focus:ring-4 focus:ring-brand-primary/10 focus:border-brand-primary outline-none transition-all"
                            ...
                            <div class="space-y-3">
                                ...
                                <select v-model="form.exam_category_id"
                                    class="w-full bg-slate-50 border border-slate-100 p-6 rounded-[1.5rem] text-xs font-black uppercase tracking-widest focus:bg-white focus:ring-4 focus:ring-brand-primary/10 focus:border-brand-primary outline-none transition-all cursor-pointer">
                            ...
                            <div class="space-y-3">
                                <label
                                    class="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Communication
                                    Medium (Language)</label>
                                <select v-model="form.language_id"
                                    class="w-full bg-slate-50 border border-slate-100 p-6 rounded-[1.5rem] text-xs font-black uppercase tracking-widest focus:bg-white focus:ring-4 focus:ring-brand-primary/10 focus:border-brand-primary outline-none transition-all cursor-pointer">
                                    <option :value="null" disabled>Select Language</option>
                                    <option v-for="lang in languages" :key="lang.id" :value="lang.id">{{ lang.name }}
                                    </option>
                                </select>
                            </div>
                        </div>
            ...
            <div v-if="currentStep === 2"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 relative z-10 pb-12">
                    <div v-for="skill in availableSkills" :key="skill.id" @click="toggleSkill(skill.id)"
                        ...
                        <div v-if="isSkillSelected(skill.id)"
                            class="absolute top-8 right-8 text-brand-primary animate-in zoom-in duration-500">
                            <div
                                class="w-8 h-8 rounded-full bg-brand-primary text-white flex items-center justify-center shadow-lg shadow-brand-primary/20">
                        ...
                        <div class="space-y-3 relative z-10">
                            <h4 class="text-sm font-black uppercase tracking-widest text-slate-800">{{ skill.name }}
                            </h4>
                            <div class="flex flex-col items-center space-y-1">
                                ...
                                <div v-if="isSkillSelected(skill.id)"
                                    class="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest mt-2 animate-in fade-in slide-in-from-top-1">
                                    Ready for matrixing
                                </div>
                            </div>
                        </div>
            ...
            <div v-if="currentStep === 3" class="space-y-16 animate-in fade-in slide-in-from-right-12 duration-700">
                <div
                    class="bg-brand-primary p-12 rounded-[4rem] text-white flex flex-col md:flex-row justify-between items-center shadow-2xl shadow-brand-primary/20 relative overflow-hidden">
                    ...
                    <div class="relative z-10 space-y-2 text-center md:text-left">
                        ...
                        <p class="text-[10px] font-bold text-rose-100 uppercase tracking-[0.2em] opacity-80">Configure
                            tier-specific content modules</p>
                    </div>
                    <div v-if="form.selectedSkills.length > 0"
                        class="relative z-10 mt-8 md:mt-0 px-8 py-3 bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 text-[10px] font-black uppercase tracking-widest whitespace-nowrap">
                        Active: {{ form.selectedSkills.length }} Modalities
                    </div>
                </div>

                <div v-if="form.selectedSkills.length === 0"
                    class="bg-slate-50 rounded-[4rem] border-2 border-dashed border-slate-200 p-32 text-center group transition-colors hover:border-indigo-300">
                    <div
                        class="w-24 h-24 bg-white rounded-[2rem] flex items-center justify-center text-5xl mx-auto mb-10 shadow-sm transition-transform group-hover:scale-110 duration-500">
                        ðŸš«</div>
                    <p class="text-sm font-black text-slate-400 uppercase tracking-[0.2em]">No modalities selected.
                        Return to sequence 2.</p>
                    <button @click="currentStep = 2"
                        class="mt-8 text-brand-primary text-[10px] font-black uppercase tracking-widest underline underline-offset-8">Configure
                        Skills Matrix âžœ</button>
                </div>

                <div v-else class="space-y-12">
                    <!-- Skill Selection Pills -->
                    <div
                        class="flex flex-wrap items-center gap-3 bg-slate-50 p-3 rounded-[2rem] border border-slate-100">
                        <button v-for="selected in form.selectedSkills" :key="selected.skill_id"
                            ...
                            <span class="w-2 h-2 rounded-full"
                                :class="activeSkillTab === selected.skill_id ? 'bg-rose-300' : 'bg-slate-200'"></span>
                    ...
                    <div v-if="activeSkillTab" class="animate-in fade-in slide-in-from-top-4 duration-500">
                        <div
                            class="bg-white rounded-[3.5rem] border border-slate-100 shadow-[0_32px_80px_-20px_rgba(0,0,0,0.03)] overflow-hidden">
                            <!-- Workplace Header -->
                            <div
                                class="p-10 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-6">
                                <div class="flex items-center space-x-6">
                                    <div
                                        class="w-16 h-16 rounded-2xl bg-rose-50 text-brand-primary flex items-center justify-center text-2xl rotate-3">
                                        {{availableSkills.find(s => s.id === activeSkillTab)?.icon || 'ðŸ§ '}}
                                    </div>
                                    <div>
                                        <h4 class="text-2xl font-black text-slate-800 uppercase tracking-tight">{{
                                            availableSkills.find(s => s.id === activeSkillTab)?.name }} Protocol</h4>
                                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                                            Configure item distribution for this cognitive modality</p>
                                    </div>
                                </div>
                                    ...
                                    <div class="bg-slate-50 px-6 py-3 rounded-xl border border-slate-100">
                                        <span
                                            class="text-[10px] font-black text-brand-primary uppercase tracking-widest">{{
                                                form.selectedSkills.find(s => s.skill_id === activeSkillTab)?.duration || 30
                                            }}m Allocation</span>
                                    </div>
                                    <Button label="Tier Matrix" icon="pi pi-table" text
                                        class="text-[9px] font-black uppercase px-6"
                            ...
                            <div class="p-10 space-y-6">
                                <div v-for="level in (availableSkills.find(s => s.id === activeSkillTab)?.levels || []).filter(l => l.is_active || !isLeveledSkill(availableSkills.find(as => as.id === activeSkillTab)))"
                                    :key="level.id"
                                    ...
                                    <div class="flex items-center space-x-6 min-w-[240px]">
                                        <div
                                            class="w-12 h-12 rounded-xl bg-white group-hover:bg-brand-primary group-hover:text-white flex flex-col items-center justify-center transition-all shadow-sm">
                                            <span class="text-[7px] font-black opacity-30 leading-none">TIER</span>
                                            <span class="text-sm font-black">{{ level.level_number || 1 }}</span>
                                        </div>
                                        <div>
                                            <h5 class="text-xs font-black uppercase text-slate-800 tracking-wider">{{
                                                level.name || 'Universal Pool' }}</h5>
                                            <div class="flex items-center space-x-2 mt-1">
                                                <div class="w-1 h-1 rounded-full bg-emerald-500"></div>
                                                <span
                                                    class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{{
                                                        localQuestions[activeSkillTab]?.[level.level_number || 1]?.length ||
                                                    0 }} Items Active</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button
                                            @click="openQuestionBuilder(activeSkillTab, level.level_number || 1, false)"
                                        ...
                                        <button
                                            @click="openQuestionBuilder(activeSkillTab, level.level_number || 1, true)"
                                            class="px-5 py-3 rounded-xl bg-white border border-slate-100 text-slate-600 text-[9px] font-black uppercase tracking-widest hover:bg-slate-800 hover:text-white transition-all shadow-sm flex items-center space-x-2">
                                    ...
                                    <div v-if="localQuestions[activeSkillTab]?.[level.level_number || 1]?.length > 0">
                                        <div
                                            class="flex items-center space-x-2 bg-rose-50 px-4 py-2 rounded-xl text-brand-primary">
                                            <i class="pi pi-check-circle text-[10px]"></i>
                                            <span
                                                class="text-[9px] font-black tracking-widest uppercase">Verified</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div v-if="currentStep === 4" class="space-y-16 animate-in fade-in zoom-in-95 duration-700">
                <div class="text-center space-y-6">
                    ...
                    <p
                        ...
                        Verification of the Logic Matrix complete. Commencing final persistence protocols.</p>
                </div>
                    ...
                    <div class="bg-white p-12 rounded-[4rem] border border-slate-100 shadow-sm space-y-10">
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="flex justify-between items-center bg-slate-50 p-6 rounded-3xl">
                                ...
                                <span class="text-xs font-black text-brand-primary">{{ form.selectedSkills.length }}
                                    Modules</span>
                            </div>
                        </div>
                    </div>
            ...
            <Dialog v-model:visible="showQuestionForm" :style="{ width: '50vw' }" modal
                class="rounded-[3rem] overflow-hidden border-none shadow-2xl info-dialog"
                :breakpoints="{ '960px': '85vw', '641px': '100vw' }">
                <template #header>
                    <div class="flex items-center w-full justify-between pr-8">
                        <div class="flex items-center space-x-6">
                            <div
                                class="w-12 h-12 rounded-2xl bg-brand-primary text-white flex items-center justify-center shadow-lg">
                                <i :class="isPassageMode ? 'pi pi-book' : 'pi pi-plus-circle'" class="text-xl"></i>
                            </div>
                            <div>
                                <h3 class="text-xl font-black text-slate-800 uppercase tracking-tight">{{ isPassageMode
                                    ? 'New Passage Resource' : 'Add New Question' }}</h3>
                                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">إضافة
                                    سؤال جديد إلى الاختبار</p>
                            </div>
                        </div>
                    </div>
                </template>
                ...
                <div class="pt-6 space-y-10 max-h-[75vh] overflow-y-auto no-scrollbar pr-4 p-6">
                    ...
                    <div class="space-y-4">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">1. Choose
                            Question Type /
                            اختر نوع السؤال</label>
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div @click="newQuestion.type = 'short_answer'"
                                ...
                                <span class="text-[9px] font-black uppercase tracking-tight">Short Answer</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- 2. Instructions -->
                    <div class="space-y-4">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">2.
                            Instructions / تعليمات
                            السؤال</label>
                        <div
                            class="bg-slate-50 p-4 rounded-2xl border border-slate-100 focus-within:bg-white focus-within:ring-4 focus-within:ring-brand-primary/5 transition-all">
                            <input v-model="newQuestion.instructions" type="text"
                                class="w-full bg-transparent border-none text-[11px] font-bold text-slate-600 outline-none"
                                placeholder="e.g. Choose the correct answer from the list below...">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div class="md:col-span-3 space-y-4">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">3.
                                Question Text / نص
                                السؤال</label>
                            <div
                                class="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 focus-within:bg-white focus-within:ring-8 focus-within:ring-brand-primary/5 transition-all">
                                <textarea v-model="newQuestion.content" rows="4"
                                    class="w-full bg-transparent border-none p-0 text-sm font-bold text-slate-700 outline-none resize-none no-scrollbar"
                                    placeholder="Start typing the question content here..."></textarea>
                            </div>
                        </div>
                        <div class="space-y-4">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Points /
                                الدرجة</label>
                            <div class="bg-slate-900 p-6 rounded-[2rem] text-white text-center">
                                <InputNumber v-model="newQuestion.points" :min="1"
                                    inputClass="bg-transparent border-none p-0 font-black text-3xl text-white outline-none w-full text-center" />
                                <span
                                    class="text-[8px] font-black text-slate-400 uppercase tracking-widest block mt-2">Weight</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">3. Audio
                            Resource (Optional)
                            / ملف صوتي</label>
                        <div v-if="newQuestion.media_path"
                            class="flex flex-col space-y-4 p-6 bg-rose-50 rounded-[2rem] border border-brand-primary/5 shadow-sm">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-3">
                                    <div
                                        class="w-8 h-8 rounded-xl bg-brand-primary text-white flex items-center justify-center shadow-lg">
                                        <i :class="isAudioPlayingInModal ? 'pi pi-pause' : 'pi pi-play'"></i>
                                    </div>
                                    <span
                                        class="text-[9px] font-black text-brand-primary uppercase tracking-widest">Protocol
                                        Linked</span>
                                </div>
                                <button @click="newQuestion.media_path = null; newQuestion.media_url = null"
                                    class="w-8 h-8 rounded-lg bg-white text-rose-500 hover:bg-rose-500 hover:text-white transition-all shadow-sm">
                                    <i class="pi pi-trash text-xs"></i>
                                </button>
                            </div>
                            <audio :key="newQuestion.media_path" :src="newQuestion.media_url"
                                @play="isAudioPlayingInModal = true" @pause="isAudioPlayingInModal = false"
                                @error="handleAudioError" controls class="h-10 w-full rounded-full bg-white/50"></audio>
                        </div>
                        <div v-else class="relative group">
                            <input type="file" @change="handleMediaUpload" accept="audio/*"
                                class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
                            <div
                                class="p-6 bg-slate-50 border-2 border-dashed border-slate-100 rounded-[2rem] flex items-center justify-center space-x-4 group-hover:bg-rose-50 transition-all">
                                <div
                                    class="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-slate-300 group-hover:text-brand-primary shadow-sm transition-all">
                                    <i :class="isUploadingMedia ? 'pi pi-spinner animate-spin' : 'pi pi-upload'"
                                        class="text-lg"></i>
                                </div>
                                <p
                                    class="text-[9px] font-black uppercase text-slate-400 group-hover:text-brand-primary transition-colors">
                                    {{ isUploadingMedia ? 'Syncing...' : 'Click to add audio file' }}</p>
                            </div>
                        </div>
                    </div>

                    <!-- 4. Shared Resource Context (Bilingual) -->
                    <div v-if="isPassageMode"
                        class="bg-brand-primary p-8 rounded-[3rem] text-white shadow-2xl space-y-6 relative overflow-hidden animate-in fade-in zoom-in duration-500">
                        <div class="flex justify-between items-center relative z-10">
                            <div class="flex items-center space-x-4">
                                ...
                                <div>
                                    <label class="text-[10px] font-black uppercase tracking-widest text-rose-100">Shared
                                        Reading Context
                                        / نص القطعة المشترك</label>
                                    <p
                                        class="text-[8px] font-bold text-rose-200 mt-0.5 uppercase tracking-widest italic">
                                        Shared logic
                                        across artifacts</p>
                                </div>
                            </div>
                            <div
                                class="flex items-center space-x-3 bg-brand-accent/30 px-5 py-3 rounded-2xl border border-white/10">
                                <div class="flex items-center space-x-2 mr-4 border-r border-rose-400/30 pr-4">
                                    ...
                                    <div @click="newQuestion.passage_randomize = !newQuestion.passage_randomize"
                                        ...
                                        <div class="w-3 h-3 bg-white rounded-full shadow-sm transition-transform duration-300"
                                            :class="newQuestion.passage_randomize ? 'translate-x-4' : 'translate-x-0'">
                                        </div>
                                    </div>
                                </div>
                                <span class="text-[8px] font-black uppercase text-rose-200">Sync Limit</span>
                                <InputNumber v-model="passageLimit" :min="1" :max="20"
                                    inputClass="w-8 text-center text-[10px] font-black border-none bg-transparent text-white focus:outline-none focus:ring-0" />
                            </div>
                        </div>
                        <textarea v-model="passageContent" rows="4"
                            class="w-full bg-white/10 border border-white/10 rounded-[2rem] p-8 text-sm font-bold text-white placeholder:text-rose-300 outline-none resize-none transition-all focus:bg-white/20 relative z-10"
                            placeholder="Paste or type the shared reading text here..."></textarea>
                    ...
                    <div v-if="newQuestion.type === 'mcq'" class="space-y-6">
                        <div class="flex justify-between items-center ml-1">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">4. Choice
                                Matrix /
                                Ø§Ù„Ø§Ø®ØªÙŠØ§Ø±Ø§Øª</label>
                            <button @click="newQuestion.options.push({ option_text: '', is_correct: false })"
                        ...
                        <div class="space-y-3">
                            <div v-for="(opt, idx) in newQuestion.options" :key="idx"
                                ...
                                <input v-model="opt.option_text" type="text"
                                    class="flex-1 bg-transparent border-none text-[11px] font-bold text-slate-700 focus:ring-0 outline-none"
                                    :placeholder="'Define choice ' + (idx + 1) + '...'">

                                <div class="flex items-center space-x-4">
                                    <label
                                        class="flex items-center px-4 py-2 rounded-xl cursor-pointer transition-all border border-transparent"
                                        :class="opt.is_correct ? 'bg-emerald-500 text-white' : 'hover:bg-slate-50 border-slate-100 text-slate-300'">
                                        <span class="text-[8px] font-black uppercase tracking-widest mr-2">{{
                                    ...
                                    <button v-if="newQuestion.options.length > 2"
                                        @click="newQuestion.options.splice(idx, 1)"
                                        class="w-8 h-8 rounded-lg text-slate-200 hover:text-rose-500 transition-colors">
                    ...
                    <div v-else-if="newQuestion.type === 'true_false'" class="space-y-6">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">4. Select
                            Logical Truth /
                            Ø­Ø¯Ø¯ Ø§Ù„Ø­Ø§Ù„Ø© Ø§Ù„ØµØ­ÙŠØ­Ø©</label>
                        <div class="grid grid-cols-2 gap-6">
                            <div @click="setTrueFalseAnswer(true)"
                                ...
                                <div :class="(newQuestion.options[0] && newQuestion.options[0].is_correct) ? 'bg-white/20' : 'bg-slate-200'"
                                    class="w-16 h-16 rounded-3xl flex items-center justify-center transition-colors">
                                    <i class="pi pi-check text-2xl"></i>
                                </div>
                                <div class="text-center">
                                    <h4 class="text-xl font-black uppercase tracking-tighter">TRUE</h4>
                                    <p class="text-[9px] font-bold opacity-60 uppercase tracking-widest mt-1">
                                        ØµÙ€Ù€Ù€Ù€Ù€ÙˆØ§Ø¨</p>
                                </div>
                            </div>
                            <div @click="setTrueFalseAnswer(false)"
                                ...
                                class="p-10 rounded-[3rem] border-2 border-transparent cursor-pointer transition-all flex flex-col items-center justify-center space-y-4 group">
                                <div :class="(newQuestion.options[1] && newQuestion.options[1].is_correct) ? 'bg-white/20' : 'bg-slate-200'"
                                    class="w-16 h-16 rounded-3xl flex items-center justify-center transition-colors">
                ...
                <template #footer>
                    <div class="flex space-x-4 pt-6 p-8 border-t border-slate-50">
                        <button
                            class="flex-1 bg-brand-primary text-white font-black text-[11px] uppercase tracking-[0.2em] py-6 rounded-[2rem] shadow-xl shadow-rose-100 hover:bg-slate-900 transition-all flex items-center justify-center space-x-4 group"
                        ...
                        <button v-if="isPassageMode"
                            class="px-8 font-black text-[10px] uppercase tracking-widest text-slate-400 hover:text-rose-500 transition-colors"
            ...
            <Dialog v-model:visible="showBankModal" :style="{ width: '85vw', height: '85vh' }" maximizable modal
                class="rounded-[4rem] overflow-hidden border-none shadow-2xl creator-studio-modal">
                <template #header>
                    <div class="flex items-center w-full justify-between pr-8">
                        <div class="flex items-center space-x-6">
                            <div
                                class="w-14 h-14 rounded-[1.8rem] bg-emerald-600 text-white flex items-center justify-center shadow-2xl">
                            ...
                            <div>
                                <h3 class="text-2xl font-black text-slate-900 uppercase tracking-tighter leading-none">
                                    Global Item Bank</h3>
                                <p
                                    class="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] mt-1.5 opacity-70">
                                    Querying Central Repository // Tier {{ activeLevelForQuestions }} Protocol</p>
                            </div>
                        </div>
                    </div>
                </template>
                ...
                <div
                    class="pt-8 space-y-10 max-h-[70vh] overflow-y-auto no-scrollbar pr-4 bg-slate-50/30 p-8 rounded-[3.5rem]">
                    <!-- Integrated Search Bar & Filters -->
                    <div class="flex flex-col md:flex-row gap-6 items-center">
                        <div class="relative group flex-1">
                            <i
                                class="pi pi-search absolute left-8 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-emerald-500 transition-colors"></i>
                            <input v-model="bankSearchQuery" type="text"
                                placeholder="Scan repository for keyword, content, or item logic..."
                        ...
                        <div class="flex items-center space-x-4 bg-white px-8 py-6 rounded-[2rem] border border-slate-100 shadow-sm">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer select-none" for="hide-assigned-bank">Hide Assigned Questions</label>
                            <input type="checkbox" id="hide-assigned-bank" v-model="hideAssignedQuestions" class="w-6 h-6 accent-emerald-500 cursor-pointer" />
                        </div>
                    </div>

                    <div v-if="bankLoading" class="flex flex-col items-center py-32 space-y-6">
                        <div class="w-10 h-10 border-4 border-slate-100 border-t-emerald-600 rounded-full animate-spin">
                        </div>
                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] animate-pulse">
                            Cross-referencing
                            global indices...</p>
                    </div>
                    ...
                    <div v-else-if="filteredBankQuestions.length === 0" class="py-20 text-center space-y-4">
                        <div
                            class="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center mx-auto text-slate-300">
                            <i class="pi pi-search text-3xl"></i>
                        </div>
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">No matching artifacts
                            found in
                            repository</p>
                    </div>
                    ...
                    <div v-else class="grid grid-cols-1 gap-4">
                        <div v-for="bq in filteredBankQuestions" :key="bq.id"
                            class="group bg-white p-8 rounded-[2.5rem] border border-slate-100 hover:border-emerald-200 hover:shadow-2xl transition-all duration-500 flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
                            
                            <div class="flex-1 space-y-4 relative z-10">
                                <div class="flex items-center space-x-3">
                                    <span
                                        class="text-[8px] font-black bg-slate-50 text-slate-400 px-3 py-1 rounded-lg uppercase tracking-widest border border-slate-100 group-hover:bg-emerald-50 group-hover:text-emerald-600 group-hover:border-emerald-100 transition-colors">{{
                                        bq.type }}</span>
                                ...
                                <h5
                                    class="text-xs font-bold text-slate-700 leading-relaxed uppercase tracking-tight line-clamp-2 md:max-w-[80%]">
                                    {{ bq.content }}</h5>
                                
                                <!-- Assignment Visibility -->
                                <div class="flex flex-wrap gap-2 mt-2">
                                     <div v-if="(bq.exams_count || 0) > 0" class="flex flex-wrap gap-1.5">
                                          <div v-for="exam in bq.exams" :key="exam.id" 
                                               class="bg-amber-50 text-amber-600 border border-amber-100 px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest shadow-sm">
                                              <i class="pi pi-link mr-1.5 text-[7px]"></i>{{ exam.title }}
                                          </div>
                                     </div>
                                     <div v-else class="bg-emerald-50 text-emerald-600 border border-emerald-100 px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest opacity-60">
                                          Fresh Asset // Unassigned
                                     </div>
                                </div>
                            </div>
                            <button @click="importQuestion(bq)" :disabled="isAlreadyAdded(bq.id)"
                                ...
                                class="shrink-0 px-10 py-5 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all relative z-10">
                                {{ isAlreadyAdded(bq.id) ? 'LINKED âœ“' : 'SYNC TO EXAM ➜' }}
                            </button>

                            <div v-if="isAlreadyAdded(bq.id)" class="absolute top-4 right-4 text-emerald-500 animate-in zoom-in duration-500">
                ...
                <template #footer>
                    <div class="flex justify-center pt-8 border-t border-slate-50">
                        <Button label="Terminate Connection" icon="pi pi-times" severity="secondary" text
                            class="font-black text-[9px] uppercase tracking-widest" @click="showBankModal = false" />
                    </div>
                </template>
            </Dialog>
            ...
            <Dialog v-model:visible="showLevelModal" :header="'Matrix Tier Calibration'" :style="{ width: '85vw' }"
                modal class="rounded-[3rem] overflow-hidden border-none shadow-2xl">
                <template #header>
                    <div class="flex justify-between items-center w-full pr-12">
                        <div class="flex flex-col">
                            <h3 class="text-xl font-black text-slate-800 uppercase tracking-tight">Matrix Tier
                                Calibration</h3>
                            <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1 italic">
                                Modifying architecture for {{ editingSkill?.name }} Domain</p>
                        </div>
                        <Button label="Append Tier" icon="pi pi-plus"
                            class="bg-brand-primary border-none font-black text-[9px] uppercase tracking-widest px-8 rounded-xl shadow-lg shadow-rose-100"
                            @click="addLevel" />
                    </div>
                </template>
                    ...
                    <DataTable :value="editingLevels" class="p-datatable-sm overflow-hidden" responsiveLayout="scroll">
                        <Column header="Tier" style="width: 80px">
                            <template #body="slotProps">
                                <div
                                    class="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center font-black text-slate-500 text-[10px] uppercase">
                        ...
                        <Column header="Logical Name">
                            <template #body="slotProps">
                                <InputText v-model="slotProps.data.name"
                                    class="w-full bg-slate-50 border-none rounded-xl text-[10px] font-black uppercase p-4 focus:bg-white focus:ring-4 focus:ring-brand-primary/10 outline-none" />
                            </template>
                        </Column>
                        <Column header="Logic Range (Points)" style="width: 250px">
                            <template #body="slotProps">
                                <div
                                    class="flex items-center space-x-3 bg-slate-50 p-3 rounded-2xl w-fit border border-slate-100">
                                    <InputNumber v-model="slotProps.data.min_score"
                                        inputClass="w-12 border-none bg-transparent text-center font-black text-[10px]" />
                                    <span class="text-slate-300 font-bold">âžœ</span>
                                    <InputNumber v-model="slotProps.data.max_score"
                                        inputClass="w-12 border-none bg-transparent text-center font-black text-[10px]" />
                                </div>
                            </template>
                        </Column>
                        <Column header="Success %" style="width: 150px">
                            <template #body="slotProps">
                                <div class="bg-rose-50/50 p-2 rounded-xl flex justify-center">
                                    <InputNumber v-model="slotProps.data.pass_threshold" suffix="%"
                                        inputClass="w-16 border-none bg-transparent text-center font-black text-[11px] text-brand-primary" />
                                </div>
                            </template>
                        </Column>
                        <Column header="Administrative Actions" style="width: 100px">
                            <template #body="slotProps">
                                <Button icon="pi pi-trash" severity="danger" text rounded
                                    @click="removeLevel(slotProps.data, slotProps.index)" class="hover:bg-rose-50" />
                            </template>
                        </Column>
                    </DataTable>
                </div>
                ...
                <template #footer>
                    <div class="flex justify-end p-6 bg-slate-50/50 border-t border-slate-50 space-x-4">
                        <Button label="Discard Changes" severity="secondary" text
                            class="font-black text-[10px] uppercase tracking-widest px-8"
                            @click="showLevelModal = false" />
                        <Button label="Persist Matrix Globally" icon="pi pi-save" :loading="isSavingLevels"
                            class="bg-brand-primary border-none font-black text-[10px] uppercase tracking-widest px-10 rounded-2xl shadow-xl shadow-rose-100"
                            @click="saveLevels" />
                    </div>
                </template>
            </Dialog>
