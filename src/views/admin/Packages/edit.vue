    ...
    <AdminLayout>
        <div class="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 px-4 md:px-12 mt-6">
            ...
            <div class="flex items-center space-x-6">
                ...
                <div>
                    <h1 class="text-3xl font-black text-slate-800 tracking-tight uppercase">Update Configured Package</h1>
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Refine bundle parameters</p>
                </div>
            </div>

            <div v-if="loading" class="flex flex-col items-center justify-center py-32 space-y-4">
                <div class="w-12 h-12 border-4 border-slate-200 border-t-indigo-600 rounded-full animate-spin"></div>
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Querying Data...</p>
            </div>

            <div v-else class="bg-white rounded-[3rem] p-10 shadow-[0_32px_120px_rgba(0,0,0,0.02)] border border-slate-100">
                <form @submit.prevent="savePackage" class="space-y-8">
                    <div class="grid grid-cols-1 gap-8">
                        ...
                        <div class="space-y-3">
                            ...
                            <textarea v-model="currentPackage.description" 
                                      rows="5"
                                      class="w-full bg-slate-50 border border-slate-100 rounded-2xl p-6 text-xs text-slate-600 font-medium focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none"></textarea>
                        </div>
                    </div>
                    ...
                    <div class="pt-8 border-t border-slate-50 flex justify-end space-x-4">
                        ...
                        <Button :label="isSaving ? 'PERSISTING...' : 'COMMIT UPDATES'" 
                                :loading="isSaving"
                                type="submit"
