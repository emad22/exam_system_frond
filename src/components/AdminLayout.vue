...
<template>
  <div class="flex h-screen bg-slate-50 font-sans text-slate-900 selection:bg-indigo-100 selection:text-indigo-900 overflow-hidden">
    
    <!-- Sidebar -->
    <aside class="w-80 bg-white border-r border-slate-100 shadow-[20px_0_40px_rgba(0,0,0,0.02)] flex flex-col justify-between hidden lg:flex relative z-20">
      <div class="flex-1 flex flex-col">
          <!-- Logo Section -->
          <div class="h-28 flex items-center px-10 mb-6">
            <div class="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white text-xl font-black shadow-xl shadow-indigo-100 mr-4 transform rotate-3 hover:rotate-0 transition-transform duration-500">
            ...
            <div>
                 <h1 class="text-2xl font-black text-slate-800 tracking-tight leading-none mb-1">ArabAcademy<span class="text-indigo-600">.</span></h1>
                 <span class="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">Exam Authority</span>
            </div>
          </div>
            ...
            <template v-for="item in navigation" :key="item.name">
                <router-link 
                    ...
                    <svg :class="[isActive(item.href) ? 'text-white' : 'text-slate-300 group-hover:text-indigo-400']" 
                         class="w-5 h-5 mr-4 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="p-8 border-t border-slate-50">
          <div class="bg-slate-50 rounded-3xl p-5 mb-4 group cursor-pointer hover:bg-indigo-50 transition-colors duration-300">
               <div class="flex items-center space-x-3">
                    <div class="w-10 h-10 rounded-xl bg-gradient-to-tr from-indigo-500 to-purple-500 shadow-md"></div>
                    <div>
                         <p class="text-xs font-black text-slate-800">Admin Staff</p>
                         <p class="text-[10px] font-bold text-slate-400 group-hover:text-indigo-500 transition-colors">Management Portal</p>
                    </div>
               </div>
          </div>
          <button @click="logout" class="w-full flex items-center justify-center space-x-2 px-6 py-4 text-xs font-black text-red-500 hover:bg-red-50 rounded-2xl transition-all active:scale-95 border border-transparent hover:border-red-100">
    ...
    <main class="flex-1 flex flex-col overflow-hidden relative">
        <div class="absolute top-0 right-0 w-[500px] h-[500px] bg-indigo-100/30 rounded-full blur-3xl -mr-64 -mt-64 pointer-events-none"></div>
        ...
        <header class="h-24 bg-white/40 backdrop-blur-xl border-b border-white/20 flex items-center justify-between px-10 z-10 sticky top-0">
            <div>
                 <div class="flex items-center space-x-2 text-[10px] font-black text-indigo-500 uppercase tracking-[0.2em] mb-1">
            ...
            <div class="flex items-center space-x-6">
                 <div class="hidden md:flex space-x-4">
                      <button class="w-10 h-10 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:border-indigo-100 transition shadow-sm">
                 ...
                 <button class="bg-indigo-600 text-white font-black text-[10px] tracking-widest uppercase px-6 py-3 rounded-xl shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition active:scale-95">
                      New Action
                 </button>
            </div>
        </header>
